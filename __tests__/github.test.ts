...
test('fetchContent', async () => {
  const content = await fetchContent('kzkn', 'npm-changelog-url-action', 'action.yml', '95b4ed3dd858', token)
  expect(content).toBeTruthy()
  expect(content).toContain("'Your name here'")
})

test('Repository#rootFileEntries', async () => {
  const repo = Repository.fromUrl('https://github.com/kzkn/npm-changelog-url-action/', token)
